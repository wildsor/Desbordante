...
#include "core/algorithms/fd/functional_dependency.h"
...
ssize_t FdHash(model::FunctionalDependency const& fd) {
    return py::hash(py::make_tuple(fd.table_name, python_bindings::VectorToTuple(fd.lhs),
...
FdList FdsToList(algos::fd::LhsMaskFdView const& storage) {
    ...
    for (model::FunctionalDependency fd : storage) {
        py::object py_fd = py::cast(std::move(fd));
...
void BindFd(py::module_& main_module) {
    ...
    py::class_<LhsMaskFdView, LhsMaskFdView::OwningPointer>(fd_module, LHS_MASK_FD_VIEW_CLASS_NAME)
            ...
            .def(py::pickle(
                    ...
                    [](LhsMaskFdView const& storage) {
                        ...
                        for (std::deque<algos::fd::LhsTableMask> const& lhs_masks_attr :
                             lhs_masks) {
                            py::list py_lhs_masks_attr{lhs_masks_attr.size()};
                            Py_ssize_t i = 0;
                            for (algos::fd::LhsTableMask const& lhs_mask : lhs_masks_attr) {
                                PyList_SET_ITEM(py_lhs_masks_attr.ptr(), i,
                                                BitsetToInt(lhs_mask).release().ptr());
                                ++i;
                            }
    ...
    auto fd_algos_module = BindPrimitive<DFD, Pyro>(fd_module, &FDAlgorithm::SortedFdList,
                                                    "FdAlgorithm", "get_fds", {"DFD", kPyroName});
    ...
    BindFdAlgorithm<FUN>(fd_algos_module, "FUN");
    BindFdAlgorithm<FastFDs>(fd_algos_module, "FastFDs");
}
}  // namespace python_bindings
