...
double AFDMetricCalculator::CalculatePerValue(model::PositionListIndex const* x_pli,
    ...
    double probability = static_cast<double>(sum + unique_rows) / (x_index.size() + unique_rows);
    return probability;
}
